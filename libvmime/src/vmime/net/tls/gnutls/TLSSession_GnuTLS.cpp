...
// FIX by Elmue: Removed: #include <iostream>
...
struct TLSGlobal
{
...
#if VMIME_TRACE && GNUTLS_DEBUG
		gnutls_global_set_log_function(TLSLogFunc);
		gnutls_global_set_log_level(GNUTLS_LOGLEVEL);
#endif
	...
	{
        // ASSERT: gnutls_buffers.c:1057
        // is not a meaningfull information for the user.
        if (level < 7 && strncmp(str, "ASSERT: ", 8) == 0)
            return;
        ...
        // FIX by Elmue: Removed dependency from Console
		TRACE("GNUTLS (%d) %s", level, str);
	}
...
string TLSSession_OpenSSL::getLibraryVersion()
{
