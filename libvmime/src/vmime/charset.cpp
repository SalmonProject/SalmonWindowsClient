...
string charset::WstringToUtf8(const wstring& inUnicode)
{
    string buf;
    buf.assign((char*)inUnicode.c_str(), inUnicode.length() * sizeof(wchar_t));

    string outUtf8;
    convert(buf, outUtf8, vmime::charsets::WSTRING, vmime::charsets::UTF_8); 
    return outUtf8;
}

// FIX by Elmue:
// UTF8 to Unicode conversion
wstring charset::Utf8ToWstring(const string& inUtf8)
{
    string buf;
    convert(inUtf8, buf, vmime::charsets::UTF_8, vmime::charsets::WSTRING); 
...
wstring charset::CpToWstring(const string& in, const vmime::charset& charsetIn)
{
    string buf;
    convert(in, buf, charsetIn, vmime::charsets::WSTRING); 
