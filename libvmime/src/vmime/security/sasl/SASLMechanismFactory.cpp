...
// FIX by Elmue: Write all implemented mechanism in libgsasl-7.dll to Trace output
void SASLMechanismFactory::traceImplementedMechanisms()
{
    #if VMIME_TRACE
        std::vector <string> supMechs = getInstance()->getSupportedMechanisms();

        string s_Supported;
        for (int i=0; i<(int)supMechs.size(); i++)
        {
            string s_Mech = supMechs.at(i);
            if (!s_Mech.length())
                continue;
        ...
        const char* s8_Version = gsasl_check_version(NULL);

        TRACE("SASL All implemented mechanisms (GSASL %s): %s", s8_Version, s_Supported.c_str());
    #endif
}
